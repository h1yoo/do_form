...
var PlusMinusRow = function (options) {
    ...
    function initComponent($tr, i) {
        ...
        var checkName = "";    // check 컴포넌트 name/id 처리시 필요
        ...
        $.each($tr.find("td input"), function (k, v) {
            ...
            // radio 초기화
                ...
                if (componentId.split("_").length > 2) {
                    newId += "_" + componentId.split("_")[2];
                }
            ...
            // check 초기화
                ...
                var curCheckName = "";
                var originalName = $(v).attr("name") || "";

                if (originalName.indexOf("_") > -1) {
                    curCheckName = originalName.split("_")[0] + "_" + originalName.split("_")[1];
                }
    ...
    function resetRowNo() {
        ...
        $("#" + settings.tableId)
            .find("tr")
            .filter("." + settings.copyRowClass + ", .copiedRow")
            .each(function () {
                $(this).find("." + settings.copyRowNoClass).text(no++);
            });
    }

    function minusRow() {
        var $checkedRows = $("#" + settings.tableId + " .copiedRow").filter(function () {
        ...
        if ($("#" + settings.tableId + " ." + settings.rowspanClass)[0] !== undefined) {
            $.each($("#" + settings.tableId + " ." + settings.rowspanClass), function (k, v) {
                $(v).attr("rowspan", parseInt($(v).attr("rowspan")) - $checkedRows.length);
            });
        }
        ...
        plusCnt = Math.max(1, plusCnt - $checkedRows.length);
    ...
    render: function () {
        ...
        PlusMinusRow({
            ...
            plusRowCallback: function () {
                var $firstRow = $("#dynamic_table1 tr.copyRow1");
                var isChecked = $firstRow.find("input[type='checkbox']").prop("checked");
                var $lastRow = $("#dynamic_table1 tr.copiedRow:last");

                $lastRow.find("input[type='checkbox']").prop("checked", isChecked);
            }
        });
    },

    calculateTotal: function (row) {
        var count = this.removeCommaToNum($(row).find('.count input').val()) || 0;
        var cost = this.removeCommaToNum($(row).find('.cost input').val()) || 0;
        console.log("count , cost = ", count, cost);
        var total = count * cost;

        $(row).find('.total').text(total.toLocaleString());
    },

    removeCommaToNum: function (str) {
        if (!str) return 0;
