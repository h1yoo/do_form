...
var Integration = Backbone.View.extend({
    ...
    render : function() {
        ...
        $(".period input").on("change", function(){
          ...
          self.calDay($row);
          console.log("날짜 변경 계산 - 행 인덱스:", $row.index());
        });
    },

    /* ✅ 날짜 계산 */
      calDay : function ($row) {// 해당 행 내부에서만 input을 찾도록 제한 (Scope 지정)
        var $inputs = $row.find('.period input');
        var startDateVal = $inputs.eq(0).val();
        var endDateVal = $inputs.eq(1).val();
    
        // 값이 둘 다 비어있지 않은지 확인
        if (startDateVal && endDateVal) {
            var startDate = moment(startDateVal, "YYYY-MM-DD");
            var endDate = moment(endDateVal, "YYYY-MM-DD");
    
            // 유효성 검사 (시작일이 종료일보다 늦은 경우 등)
            if (!startDate.isValid() || !endDate.isValid() || startDate.isAfter(endDate)) {
                $row.find(".calDay input").val(""); // 유효하지 않으면 비움
                return;
            }
            ...
            // 해당 행(row)의 결과 칸에만 정확히 삽입
            $row.find(".calDay input").val(dayDiff);
            
        } else {
            $row.find(".calDay input").val(""); 
        }
    },
