...
var Integration = Backbone.View.extend({
	...
	render : function() {
		...
		$(".cur input, .amount input, .vat input").on("change",function(){self.calAmount();});
	...
	calAmount : function () {
		...
		$("#dynamic_table1 tr").each(function(i, e){
			 if ($(e).find('.amount')[0]) {
				...
				var cur = parseFloat($(e).find('.cur input').val().replace(/\,/g,"")); if (isNaN(cur)) cur = 0;
				var vat = parseFloat($(e).find('.vat input').val().replace(/\,/g,"")); if (isNaN(vat)) vat = 0;

				price = parseFloat((cur / amount).toFixed(0));
				$(e).find(".price").text(GO.util.numberWithCommas(price));
				total = parseFloat((cur + vat).toFixed(2));
				$(e).find(".total").text(GO.util.numberWithCommas(total));
				
				sum_price 	= parseFloat((sum_price+price).toFixed(2));
				sum_vat 	= parseFloat((sum_vat+vat).toFixed(2));
				sum_total 	= parseFloat((sum_total+total).toFixed(2));
			}
		});
		//$(".sum_price").text(GO.util.numberWithCommas(sum_price));
		//$(".sum_vat").text(GO.util.numberWithCommas(sum_vat));
		$(".sum_total").text(GO.util.numberWithCommas(sum_total));
		$('#sum_total_kor').text(self._viewKorean(sum_total.toString()));
	},
