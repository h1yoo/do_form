...
var Integration = Backbone.View.extend({
    ...
    $(".cost input").on("change",function(){self.calAmount();});
    ...
    var price = 0;
       ...
       if ($(e).find('.price')[0]) {
        ...
        var cost = parseFloat($(e).find('.cost input').val().replace(/\,/g,"")); if (isNaN(cost)) cost = 0;//단가

        price = parseFloat((amount * cost).toFixed(2));
        $(e).find(".price").text(self._convertCurrencyFormat(price));
  ...
  //마법의한줄 - 어디서든 값을 가져올수있단다 (ID로 부여할것 Class는 안대!)
  price = (document.getElementById("price").getElementsByTagName("input")[0].value) == "" ? 0 : ((document.getElementById("price").getElementsByTagName("input")[0].value).replace(",",""))
