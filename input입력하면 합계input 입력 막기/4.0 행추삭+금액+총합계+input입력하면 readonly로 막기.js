...
var Integration = Backbone.View.extend({
	...
	render : function() {
		...
		PlusMinusRow({
				...
				copyRowNoClass : "rowNo1",
				rowspanClass : "rowspanTd1",
        minusRowCallback : function() {},
        plusRowCallback : function() {}
    });
    
    // $(".amount input, .unit input, .price input, .taxable_other_charges input").on("change",function(){
    //   self.calPrice();
    //   self.calSumPrice();

    //   // .sub_total input, .grand_total input readonly 처리 >> 이렇게 안돼서 아래와 같이 코드 수정
    //   if ($(this).val() != "") {
    //     $(".sub_total input").prop("readonly", true);
    //     $(".grand_total input").prop("readonly", true);
    //   } else {
    //     $(".sub_total input").prop("readonly", false);
    //     $(".grand_total input").prop("readonly", false);
    //   }
    ...
    //   }
    // });
    $(document).on("change input", ".amount input, .unit input, .price input, .taxable_other_charges input", function(e) {
    
      // 계산 함수 호출
      if (typeof self.calPrice === "function") self.calPrice();
      if (typeof self.calSumPrice === "function") self.calSumPrice();
  
      // 2. [개선] 문서 내의 모든 대상 input 중 하나라도 값이 입력되어 있는지 체크
      var hasValue = false;
      $(".amount input, .unit input, .price input, .taxable_other_charges input").each(function() {
          if ($(this).val().trim() !== "") {
              hasValue = true;
              return false; // 루프 탈출
          }
      ...
      // 하나라도 입력되어 있다면 readonly 설정, 모두 비어있으면 해제
      $(".sub_total input, .grand_total input").prop("readonly", hasValue);
  
  
      // 3. 콤마(포맷팅) 로직 처리 
      // (사용자가 입력 중인 창은 change 이벤트일 때만 콤마를 찍어야 커서가 안 튑니다)
      if (e.type === "change") {
          let value = $(this).val().replace(/,/g, "");
          if (!isNaN(value) && value !== "") {
  ...
  calPrice : function () {
    ...
    $("#dynamic_table1 tr").each(function(i, e){
      if ($(e).find(".amount")[0]) {
        ...
        var unit = parseInt($(e).find(".unit input").val().replace(/\,/g,"")); if (isNaN(unit)) unit = 0;
        var price = amount * unit || 0;
        
        sum_price += price;
        
        $(e).find(".price input").val(GO.util.numberWithCommas(price));
      }
    });
    
    var taxable_other_charges = parseInt($(".taxable_other_charges input").val().replace(/\,/g,"")); if (isNaN(taxable_other_charges)) taxable_other_charges = 0;

    var sub_total = sum_price + sum_price * 0.1 || 0;
    var grand_total = taxable_other_charges + sub_total || 0;
    
    $(".sub_total input").val(GO.util.numberWithCommas(sub_total.toFixed(0)));
    $(".grand_total input").val(GO.util.numberWithCommas(grand_total.toFixed(0)));
    $(".paste_grand_total").text(grand_total.toLocaleString());
  },
