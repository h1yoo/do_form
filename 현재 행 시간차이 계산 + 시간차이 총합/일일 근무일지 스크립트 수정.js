...
// /* ========================= 시간 자동계산 유틸 ========================= */
// function parseTimeToMin(h, m) {
//   var hh = parseInt(h, 10); if (isNaN(hh)) hh = 0;
//   var mm = parseInt(m, 10); if (isNaN(mm)) mm = 0;
//   return hh * 60 + mm;
// }

// function formatMinToHM(mins) {
...
// function calcRowDurationMin($tr) {
//   var sh = $tr.find(".start_h").val();
//   var sm = $tr.find(".start_m").val();
//   var eh = $tr.find(".end_h").val();
//   var em = $tr.find(".end_m").val();

//   if (sh === undefined || sm === undefined || eh === undefined || em === undefined) return 0;

//   var start = parseTimeToMin(sh, sm);
//   var end = parseTimeToMin(eh, em);

//   var diff = end - start;
//   if (diff < 0) diff += 24 * 60; // 종료 < 시작이면 익일 처리
//   return diff;
// }

// function setRowDuration($tr, minutes) {
//   // 화면 표기는 "n시간 m분"
//   var $input = $tr.find('span[data-dsl="{{number:durationMin}}"] input');
//   if ($input.length) {
//     $input.val(formatMinToHM(minutes));
//     // 숫자(분)는 data 속성으로 보관(필요시 활용)
//     $input.attr("data-minutes", String(minutes));
//     $input.trigger("change");
//   }
// }

// function setTotal(totalMinutes) {
//   var $input = $('span[data-dsl="{{number:totalMin}}"] input');
//   if ($input.length) {
//     $input.val(formatMinToHM(totalMinutes));
//     $input.attr("data-minutes", String(totalMinutes));
//     $input.trigger("change");
//   }
// }

// function recalcAll() {
//   var $tbody = $("#dynamic_table1");
//   var $rows = $tbody.find("tr.copyRow1, tr.copiedRow");
//   var total = 0;

//   $rows.each(function () {
//     var $tr = $(this);
//     var dur = calcRowDurationMin($tr);
//     setRowDuration($tr, dur);
//     total += dur;
//   });
...
var Integration = Backbone.View.extend({
  ...
  render: function () {
    ...
    PlusMinusRow({
      ...
      // 행 추가/삭제 후 합계 재계산
      // plusRowCallback: function () { recalcAll(); },
      // minusRowCallback: function () { recalcAll(); }
      plusRowCallback: function () { self.calTime(); },
      minusRowCallback: function () {}
    });
    
    $(".start_h, .start_m, .end_h, .end_m").on("change", function(){
      self.calTime($(this).closest('tr'));  // self, this 일부러 구분함 > 수정금지
      self.calSumTime();
    });
  },
    
  calTime : function ($row) {// 해당 행 내부에서만 input을 찾도록 제한 (Scope 지정)
    var startH = Number($row.find('.start_h').val()) || 0;
    var startM = Number($row.find('.start_m').val()) || 0;
    var endH = Number($row.find('.end_h').val()) || 0;
    var endM = Number($row.find('.end_m').val()) || 0;
    ...
    if (diff >= 0) {
      ...
      $row.find(".cal_time input").val(h + "시간 " + m + "분");
    }
  },
  ...
  calSumTime : function () {
    ...
    $(".cal_time input").each(function () {
      var text = $(this).val(); // 예: "2시간 30분"
      if (!text) return;
  
      // 시간, 분 추출
      var hMatch = text.match(/(\d+)\s*시간/);
      var mMatch = text.match(/(\d+)\s*분/);
      console.log("?!@#!#?!@>!: ", hMatch, mMatch);
    ...
    var finalH = Math.floor(totalMinutesSum / 60);
    var finalM = totalMinutesSum % 60;
  
    $(".cal_sum_time input").val(finalH + "시간 " + finalM + "분");    
  },
