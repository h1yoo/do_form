...
var Integration = Backbone.View.extend({
    ...
    PlusMinusRow({
        ...
        rowspanClass : "rowspanTd3",
				minusRowCallback : function() {self.calCost();},
				plusRowCallback : function() {self.calCost();}
    });
    PlusMinusRow({
		...
		$(".accomCost input, .transCost input, .foodCost input").on("change",function(){
      self.calCost();
  ...
  calCost : function () {
    ...
    $("#dynamic_table3 tr").each(function(i, e){
      if ($(e).find(".sumCost")[0]) {
        var accomCost = parseInt($(e).find(".accomCost input").val().replace(/,/g, "")) || 0;
        var transCost = parseInt($(e).find(".transCost input").val().replace(/,/g, "")) || 0;
        var foodCost = parseInt($(e).find(".foodCost input").val().replace(/,/g, "")) || 0;
        
        sumCost = accomCost + transCost + foodCost;
        totalCost += sumCost;

        $(e).find(".sumCost").text(GO.util.numberWithCommas(sumCost.toFixed(0)));
      }
    });
