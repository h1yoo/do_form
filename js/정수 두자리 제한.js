...
var Integration = Backbone.View.extend({
    ...
    $(".existingMM input").on("change",function(){
      self.calPrice(".existingMM input", ".sum_existingMM");
    });
    
    $(".changeMM input").on("change",function(){
      self.calPrice(".changeMM input", ".sum_changeMM");
    });

    // existingMM, changeMM 인풋 입력 시 키보드 차단
    $(document).on("keydown", ".existingMM input, .changeMM input", function(e) {
      var key = e.key;
      var val = $(this).val();

      // 지우기(Backspace), 탭(Tab), 화살표, Delete 키 등 제어용 키는 허용
      ...
      // 숫자 또는 소수점(.) 입력인 경우 검사
      if (/^[0-9.]$/.test(key)) {
        // 이미 소수점이 있는데 또 . 을 누르면 차단
        if (key === '.' && val.includes('.')) {
          e.preventDefault();
          return false;
        }

        // 현재 커서 위치 기준으로 소수점 앞(정수부) 추출
        var parts = val.split('.');
        
        // 소수점을 찍기 전 상태이고, 이미 정수가 2자리라면 3번째 숫자 입력 차단
        if (parts.length === 1 && val.length >= 2 && key !== '.') {
          e.preventDefault();
          return false;
        }
      } else {
        // 숫자가 아닌 문자(영문, 특수문자 등) 입력 차단
        e.preventDefault();
        return false;
      }
    });

    // 복사/붙여넣기(Paste)로 3자리 이상 들어오는 경우도 강제 차단
    $(document).on("paste", ".existingMM input, .changeMM input", function(e) {
      var pastedData = (e.originalEvent || e).clipboardData.getData('text');
      var parts = pastedData.split('.');
      
      // 붙여넣은 정수 부분이 2자리를 넘으면 붙여넣기 취소
      if (parts[0].replace(/[^0-9]/g, '').length > 2) {
        e.preventDefault();
        alert("정수는 최대 2자리까지만 입력 가능합니다.");
      }
    });
	},
  
  calPrice : function (prcieEL, sumPriceEL) {
    var sum_existingMM = 0;
    var sum_changeMM = 0;

    $("#dynamic_table1 tr").each(function(i, e){
      if ($(e).find(".existingMM")[0]) {
        var existingMM = parseFloat($(e).find(".existingMM input").val().replace(/\,/g,"")); if (isNaN(existingMM)) existingMM = 0;
        
        sum_existingMM += existingMM;
      }

      if ($(e).find(".changeMM")[0]) {
        var changeMM = parseFloat($(e).find(".changeMM input").val().replace(/\,/g,"")); if (isNaN(changeMM)) changeMM = 0;
        
        sum_changeMM += changeMM;
      }
    });
    
    if (prcieEL.includes("existingMM")) {
      $(sumPriceEL).text(GO.util.numberWithCommas(sum_existingMM.toFixed(2)));
      
    } else if (prcieEL.includes("changeMM")) {
      $(sumPriceEL).text(GO.util.numberWithCommas(sum_changeMM.toFixed(2)));
    }
  },
