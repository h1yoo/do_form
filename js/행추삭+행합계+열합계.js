...
var Integration = Backbone.View.extend({
	...
	render : function() {
     ...
     PlusMinusRow({
      ...
      $("#dynamic_table2 input").on("change",function(){
          self.calAmount();
        });
	},
    ...
    calAmount : function () {
      ...
      var sum_fuelCost = 0;
      var sum_etcCost = 0;
      var sum_total1person = 0;
      
      $("#dynamic_table2 tr").each(function(i, e){
        if ($(e).find(".transCost")[0]) {
          var transCost = parseFloat($(e).find(".transCost input").val().replace(/,/g, "")) || 0;
          var mealCost = parseFloat($(e).find(".mealCost input").val().replace(/,/g, "")) || 0;
          var accomCost = parseFloat($(e).find(".accomCost input").val().replace(/,/g, "")) || 0;
          var dayCost = parseFloat($(e).find(".dayCost input").val().replace(/,/g, "")) || 0;
          var fuelCost = parseFloat($(e).find(".fuelCost input").val().replace(/,/g, "")) || 0;
          var etcCost = parseFloat($(e).find(".etcCost input").val().replace(/,/g, "")) || 0;
          
          var total1person = 0;
          total1person = transCost + mealCost + accomCost + dayCost + fuelCost + etcCost;

          $(e).find(".total1person").text(GO.util.numberWithCommas(total1person.toFixed(0)));

          sum_transCost += transCost;
          sum_mealCost += mealCost;
          sum_accomCost += accomCost;
          sum_dayCost += dayCost;
          sum_fuelCost += fuelCost;
          sum_etcCost += etcCost;
          sum_total1person += total1person;
        }
      });
      
      $(".sum_transCost").text(GO.util.numberWithCommas(sum_transCost));
      $(".sum_mealCost").text(GO.util.numberWithCommas(sum_mealCost));
      $(".sum_accomCost").text(GO.util.numberWithCommas(sum_accomCost));
      $(".sum_dayCost").text(GO.util.numberWithCommas(sum_dayCost));
      $(".sum_fuelCost").text(GO.util.numberWithCommas(sum_fuelCost));
      $(".sum_etcCost").text(GO.util.numberWithCommas(sum_etcCost));
      $(".sum_total1person").text(GO.util.numberWithCommas(sum_total1person));
    },
