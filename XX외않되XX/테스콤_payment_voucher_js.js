...
var Integration = Backbone.View.extend({
	...
	render : function() {
		var self = this;
		$(".sel_vat select, .sel_cur select, .amount input, .price input").on("change",function(){
      self.calAmount();
      self.calABCDE();
      self.calPercent();
    });
		$(".sum_cur, .calIGST_B input, .calCGST_C input, .calSGST_D input, .calTDS_E input").on("change",function(){
      self.calABCDE();
      self.calPercent();
    });
		$(".advPayInput input").on("change",function(){self.calPercent();});
