...
var Integration = Backbone.View.extend({
    ...
    // 초기 렌더 시 통화별 금액 총합계 숨기기
    $(".totalDollarPrice").hide();
    $(".totalEuroPrice").hide();
    $(".totalYenPrice").hide();
    $(".totalWonPrice").hide();
      ...
      plusRowCallback : function() {}
		...
		$(".amount input, .unit input, .price input").on("change",function(){
      self.calPrice();
      self.calTotalPrice();
  ...
  calPrice : function () {
    $("#dynamic_table1 tr").each(function(i, e){
      if ($(e).find(".amount")[0]) {
        var amount = parseFloat($(e).find(".amount input").val().replace(/,/g, "")) || 0;
        var unit = parseFloat($(e).find(".unit input").val().replace(/,/g, "")) || 0;
        ...
        $(e).find(".price input").val(GO.util.numberWithCommas(price.toFixed(2)));
      }
    });
  },
  ...
  calTotalPrice : function () {
    ...
    $(".price input").each(function () {
      ...
      var curText = $(this).closest("tr").find(".cur select option:selected").val();
  
      if (curText  == "달러") {
        totalDollarPrice += val;
      }
      if (curText == "유로") {
        totalEuroPrice += val;
      }
    ...
    // 달러
    if (totalDollarPrice != 0) {
      console.log("totalDollarPrice:", totalDollarPrice);
      $(".totalDollarPrice").show();
      $(".totalDollarPrice").text("$" + GO.util.numberWithCommas(totalDollarPrice.toFixed(2)));
    } else {
      $(".totalDollarPrice").hide();
    }
    ...
    // 유로
    if (totalEuroPrice != 0) {
      console.log("totalEuroPrice:", totalEuroPrice);
      $(".totalEuroPrice").show();
      $(".totalEuroPrice").text("€" + GO.util.numberWithCommas(totalEuroPrice.toFixed(2)));
    } else {
      $(".totalEuroPrice").hide();
    }
    ...
    // 엔화
    if (totalYenPrice != 0) {
      console.log("totalYenPrice:", totalYenPrice);
      $(".totalYenPrice").show();
      $(".totalYenPrice").text("¥" + GO.util.numberWithCommas(totalYenPrice.toFixed(2)));
    } else {
      $(".totalYenPrice").hide();
    }
    ...
    if (totalWonPrice != 0) {
      ...
      $(".totalWonPrice").show();
      $(".totalWonPrice").text("₩" + GO.util.numberWithCommas(totalWonPrice.toFixed(0)));
    } else {
      $(".totalWonPrice").hide();
    }
  },
  ...
  checkOpt : function(){
      ...
      var checkMS = $('.checkMS input:checked').val();  // 구분 - 민수 체크박스가 체크됐는지 확인할 변수
      var checkBushing1 = $('.checkBushing1 input:checked').val();  // 1차부싱 체크박스가 체크됐는지 확인할 변수
      var checkBushing2 = $('.checkBushing2 input:checked').val();  // 2차부싱 체크박스가 체크됐는지 확인할 변수
      var checkBushingDirec = $('.checkBushingDirec input:checked').val();  // 2차부싱 방향 체크박스가 체크됐는지 확인할 변수

      if(checkMS != undefined){
        if(checkBushing1 == undefined || checkBushing2 == undefined || checkBushingDirec == undefined){
          throw new Error("1차부싱, 2차부싱, 2차부싱 방향 체크 필수");
        }
      }
  },
