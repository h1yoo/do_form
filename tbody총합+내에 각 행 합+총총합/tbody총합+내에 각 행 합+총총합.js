...
var PlusMinusRow = function (options) {
  ...
  function plusRow() {
    ...
    // 자동 계산 텍스트 초기화
    $tr.find(".price, .sum_amount, .sum_price").text("");  // 행추가 시 초기 텍스트 제거
...
var Integration = Backbone.View.extend({
    ...
    $(".transPrice input, .accomPrice input, .amount input").on("change",function(){
      self.calPrice();
  ...
  calPrice : function () {
    ...
    $("#dynamic_table1 tbody").each(function(i, e){
      var sum_amount = 0;
      var sum_other_price = 0;
      var sum_only_price = 0;

      // 1. 우선 해당 tbody(e) 안에 들어있는 교통비와 숙박비를 싹 긁어 모읍니다.
      if ($(e).find(".transPrice input").length > 0) {
        var transPrice = parseInt($(e).find(".transPrice input").val().replace(/\,/g,"")) || 0;
        var accomPrice = parseInt($(e).find(".accomPrice input").val().replace(/\,/g,"")) || 0;
        
        sum_other_price += (transPrice + accomPrice);
      }

      // 현재 tbody(e) 내부의 '각 행(tr)'들을 돌면서 개별 price를 계산
      $(e).find("tr").each(function(idx, row) {
        if ($(row).find(".amount").length > 0) {
          var amount = parseInt($(row).find(".amount input").val().replace(/\,/g,"")) || 0;
          var val = parseInt($(row).find(".val").text().replace(/[^0-9,]/g, "").replace(/,/g, "")) || 0;

          // 현재 행의 순수 물품 금액 계산
          var price = amount * val;
          
          // 계산된 금액을 현재 행(row)의 .price 자리에 정확히 주입
          $(row).find(".price").text(GO.util.numberWithCommas(price));

          sum_amount += amount;
          sum_only_price += price;
        }
      });
      
      var sum_price = sum_only_price + sum_other_price;
      total_price += sum_price;
      
      $(e).find(".sum_amount").text(GO.util.numberWithCommas(sum_amount));
      $(e).find(".sum_price").text("￦" + GO.util.numberWithCommas(sum_price));
    });
