...
var Integration = Backbone.View.extend({
	...
	render : function() {
		...
		$(".prePrice1 input, .won1 input, .foreign1 input").on("change",function(){
            self.calOverPrice1(".prePrice1", ".prePrice1 input", ".won1 input", ".overPrice1");

            self.calSumPrice('#corpCardCost .prePrice1 input', '.sum_prePrice1');
            self.calSumPrice('#corpCardCost .won1 input', '.sum_won1');
            self.calSumPrice('#corpCardCost .foreign1 input', '.sum_foreign1');

            self.calSumPriceText('#corpCardCost .totalWon1', '.sum_totalWon1');
            self.calSumPriceText('#corpCardCost .overPrice1', '.sum_overPrice1');
        });

        $('.exchangeforeign1 select').on('change', function() {
            var selectedValue = $(this).val();
            $('.selectedforeign1').text(selectedValue ? selectedValue : '');
        });
        
        $('.exchangeforeign2 select').on('change', function() {
            var selectedValue = $(this).val();
            $('.selectedforeign2').text(selectedValue ? selectedValue : '');
        });
	},

	calOverPrice1 : function (prePriceEl, prePriceValEl, wonValEl, overPriceEl) {
		var self = this;
		...
		$("#corpCardCost tr").each(function(i, e){
            if ($(e).find(prePriceEl)[0]) {
                ...
                $(e).find(".totalWon1").text(GO.util.numberWithCommas(price));
			}
		});
	},
	...
	calExWon : function (exRateEl, foreignPriceValEl, calWonEl) {
		var self = this;
	...
	calOverPrice2 : function (prePriceEl, prePriceValEl, wonValEl, overPriceEl) {
		var self = this;
