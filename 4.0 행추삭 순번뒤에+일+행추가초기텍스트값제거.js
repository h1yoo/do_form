...
var PlusMinusRow = function (options) {
  ...
  function plusRow() {
    ...
    // 식비 소계 > 자동 계산 부분 초기화
    $tr.find(".subtotal").text("");  // 초기 텍스트 제거
    ...
    //     $($tr.find("." + settings.copyRowNoClass)[i]).text(newNo + "일");
    //   }
    // }
        // // ////////////// 순번처리 코드 수정 > 텍스트 제거 후 추가
        if ($("#" + settings.tableId + " ." + settings.copyRowNoClass)[0] !== undefined) {
          var copyRowNoCnt = $tr.find("." + settings.copyRowNoClass).length; // 한 행에 존재하는 순번 클래스의 수
...
var Integration = Backbone.View.extend({
	...
	render : function() {
        ...
        minusRowCallback : function() {
          self.calCost();
          self.calSumAmount(".excAmount input", ".sum_excAmount");
        },
        plusRowCallback : function() {}
    });
    
		$(".foodExp1 input, .foodExp2 input, .foodExp3 input").on("change",function(){
      self.calCost();
      ...
      if (!isNaN(value) && value !== "") {
		...
		$(".excAmount input").on("change",function(){
      self.calSumAmount(".excAmount input", ".sum_excAmount");
  ...
  calCost : function () {
    ...
    $(".sum_subtotal").text(GO.util.numberWithCommas(sum_subtotal));
  },

	calSumAmount : function (inputEl, sumEl) {
		var self = this;
		var sum_amount = 0;

    $(inputEl).each(function(i,e){
    // $('.amount input').each(function(i,e){
      sum_amount += Number($(e).val().replace(/,/g, ""));
    })
    
		$(sumEl).text(GO.util.numberWithCommas(sum_amount));
		// $(".sum_amount").text(GO.util.numberWithCommas(sum_amount));
	},
