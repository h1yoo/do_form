...
var Integration = Backbone.View.extend({
    ...
    self.checkSelected(); // 기안일자가 23일 이후인지 체크하기 위해 렌더 시 checkSelected 함수 호출

    $(".check1 input, .period input").on("change",function(){
      self.checkSelected();
    });
	},

  checkSelected : function(){
    // 기안일 가져오기 및 초기 기본값 세팅
    var draftDateVal = $("#draftDate").val() || moment().format("YYYY-MM-DD");
    console.log("draftDateVal : ", draftDateVal);
    
    // // moment 객체는 상단에서 한 번만 깔끔하게 선언해서 재사용합니다. (오타 및 중복 선언 해결)
    // var draftDate = moment(draftDateVal, "YYYY-MM-DD"); 

    // 선택된 시작일 가져오기
    var startDateVal = $(".period input").eq(0).val() || "";

    // 선택된 종료일 가져오기
    var endDateVal = $(".period input").eq(1).val() || "";
    var endDate = moment(endDateVal, "YYYY-MM-DD");
    
    // 단타특매 or 고정특매 품목추가 (단일선택 값 체크)
    var check1 = $('.check1 input:checked').val() || ""; // 아무것도 체크 안 되었을 때를 대비해 빈값("") 처리

    // includes 에러 방지용 안전장치 추가
    if(check1 && check1.includes("단타특매")){
      // 기안일이 '23일 이후'인지 체크 (24일~말일)
      if (draftDateDay > 23) {
        // 선택한 날짜의 '일'이 23보다 크면 해당 월의 말일로 자동 강제 전환
        $(".period input").eq(1).val(lastDayOfMonth);
        
        // console.log("23일 이후 선택으로 인해 말일(" + lastDayOfMonth + ")로 자동 전환됨");
      }

      if (startDateVal && endDateVal && endDate.isValid()) {
        var draftDate = moment(draftDateVal, "YYYY-MM-DD");
        
        // [조건] 시작일 기준 최대 30일 마지노선 계산
        var maxAllowedDate = moment(startDateVal, "YYYY-MM-DD").add(30, 'days');
        
        // [조건] 종료일이 속한 달의 '일(Day)' 계산
        var draftDateDay = draftDate.date(); // 예: 2026-07-25 이면 25 반환

        // 기안 월의 말일로 설정
        var lastDayOfMonth = moment(draftDateVal, "YYYY-MM-DD").endOf('month').format("YYYY-MM-DD");

        // 시작일 기준 30일을 초과했는지 체크
        if (endDate.isAfter(maxAllowedDate)) {
          alert("단타특매 선택 시 종료일은 시작일로부터 최대 30일 이내까지만 설정할 수 있습니다.\n(최대 선택 가능일: " + maxAllowedDate.format("YYYY-MM-DD") + ")");
          
          $(".period input").eq(1).val(lastDayOfMonth); // 30일 초과 시 기안 월의 말일로 설정
        }
      }
    } else {
      // 단타특매가 아닐 때는 날짜 제한을 풀어줍니다 (단, datepicker가 바인딩되어 있을 때만 호출)
      if($.fn.datepicker && $(".period input").eq(1).data('datepicker')) {
          $(".period input").eq(1).datepicker("option", "maxDate", null);
      }
    }
  },
