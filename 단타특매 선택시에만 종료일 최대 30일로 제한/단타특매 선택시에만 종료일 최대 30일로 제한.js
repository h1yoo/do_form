...
var Integration = Backbone.View.extend({
  ...
  checkSelected : function(){
    ...
    // 3. 단타특매 or 고정특매 품목추가 (단일선택 값 체크)
    var check1 = $('.check1 input:checked').val() || ""; // 아무것도 체크 안 되었을 때를 대비해 빈값("") 처리

    // includes 에러 방지용 안전장치 추가
    if(check1 && check1.includes("단타특매")){
      
      // 기안일 기준 최대 30일 마지노선 계산
      var maxAllowedDate = moment(draftDateVal, "YYYY-MM-DD").add(30, 'days');
      
      // 초과 선택 시 대체 입력될 '기안월의 말일' 구하기
      var lastDayOfMonth = moment(draftDateVal, "YYYY-MM-DD").endOf('month').format("YYYY-MM-DD");

      if (endDateVal && endDate.isValid()) {
        
        // 비교 분석 (종료일이 기안일+30일 후보다 더 미래인지 확인)
        if (endDate.isAfter(maxAllowedDate)) {
          alert("단타특매 선택 시 종료일은 기안일로부터 최대 30일 이내까지만 설정할 수 있습니다.\n(최대 선택 가능일: " + maxAllowedDate.format("YYYY-MM-DD") + ")");
          
          // 초과 선택 시 종료일을 해당 월 말일로 자동 입력
