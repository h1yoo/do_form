...
var Integration = Backbone.View.extend({
	...
	render : function() {
      ...
      plusRowCallback : function() {}
		...
		$(".amount input, .unitPrice input").on("change",function(){
      ...
      if (!isNaN(value) && value !== "") {
		...
		$(".tt_price input, .tt_tax input").on("change",function(){
      var tt_price = parseFloat($(e).find('.tt_price input').val().replace(/\,/g,""));
      if (isNaN(tt_price) || tt_price == 0) tt_price = 0;
  ...
  calPrice : function ($row) {
    if (!$row || $row.length === 0) return;
  
    var amount = parseFloat($row.find('.amount input').val().replace(/,/g,""));
    if (isNaN(amount)) amount = 0;
  
    var unitPrice = parseFloat($row.find('.unitPrice input').val().replace(/,/g,""));
    if (isNaN(unitPrice)) unitPrice = 0;
  
    var price = amount * unitPrice;
    var tax = price * 0.1;
  
    $row.find(".price input").val(GO.util.numberWithCommas(price.toFixed(0)));
    $row.find(".tax input").val(GO.util.numberWithCommas(tax.toFixed(0)));
  },
    ...
    $(".tt_price input").val(GO.util.numberWithCommas(tt_price));
    $(".tt_tax input").val(GO.util.numberWithCommas(tt_tax));
	},
