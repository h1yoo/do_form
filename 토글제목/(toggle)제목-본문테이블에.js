...
var Integration = Backbone.View.extend({
    ...
    render : function() {
        ...
        $("#tmp_title, #tmp_title .ipt_editor").on("change",function(){
            self.setDocTit();
        });
    },	

    setDocTit : function() {
        var copyTitInput = $("#subject").val(); // 입력한 문서 제목 가져오기        
        $(".docTit").text(copyTitInput); // 페이지 로드 시 초기값 설정
    },
    ...
    beforeSave :function() {
      ...
      // 사용구분에 따라 제목 앞에 기본값 설정
      var temp = $('.writeTitle input').val();
      var select = $('.use_toggle input:checked').val();
      $('#subject').val("["+select+" 신청서] "+temp);
      
      var copyTitInput = $("#subject").val(); // 입력한 문서 제목 가져오기        
      $(".docTit").text(copyTitInput); // 페이지 로드 시 초기값 설정

      $('.writeTitle input').val($('#subject').val());
    },
