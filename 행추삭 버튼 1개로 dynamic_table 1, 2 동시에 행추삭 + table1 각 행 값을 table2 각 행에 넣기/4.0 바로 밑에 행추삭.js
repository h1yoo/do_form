...
var Integration = Backbone.View.extend({
    ...
    // 1. [플러스 버튼 연동] #plus1을 누르면 #plus2도 같이 누른 효과
    $(document).off("click", "#plus1");
    $(document).on("click", "#plus1", function(e) {
      // 1번 테이블은 원래대로 추가 기능이 실행됨
      // 그 직후, 2번 테이블의 진짜 추가 버튼(#plus2)을 강제로 클릭 처리
      $("#plus2").trigger("click");
    });

    // 2. [마이너스 버튼 연동] #minus1을 누르면 #minus2도 같이 누른 효과
    $(document).off("click", "#minus1");
    $(document).on("click", "#minus1", function(e) {
      // 1번 테이블 행 삭제 실행 직후
      // 2번 테이블의 삭제 버튼(#minus2)도 강제로 트리거(클릭)
      $("#minus2").trigger("click");
    });

    // 1번 테이블의 .repDate input 값이 바뀔 때(입력하거나 변경할 때) 작동
    $("#dynamic_table1 .repDate input").on("change", function() {
        
      // 현재 값이 바뀐 input이 1번 테이블에서 '몇 번째' 행에 있는지 순서(index)를 찾습니다.
      // (동일한 클래스를 가진 전체 input 중 현재의 순서)
      var currentIndex = $("#dynamic_table1 .repDate input").index(this);
      
      // 현재 입력된 값을 가져옵니다.
      var currentVal = $(this).val();
      
      // 2번 테이블에서 '동일한 순서(eq)'에 있는 .repDate input을 정확히 조준해서 값을 똑같이 주입합니다.
      $("#dynamic_table2 .repDate input").eq(currentIndex).val(currentVal);
    });
	},
