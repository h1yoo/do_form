...
var PlusMinusRow = function (options) {
  ...
  function initComponent($tr, i) {
    ...
    ////// 부서 & 이름 추가된 값 초기화 코드 추가 START //////
    // name_tag 초기화
    $.each($tr.find(".name_tag"), function (k, v) {
      // 기존 선택값 초기화 (원하면 유지 가능)
      $(v).attr("data-selected", "");
      $(v).find("li").not(".creat").remove();
    });
    ////// 부서 & 이름 추가된 값 초기화 코드 추가 END //////
...
var Integration = Backbone.View.extend({
	...
	render : function() {
		...
		// // 텍스트 박스 readonly 처리 > html에 적용 시 안 먹어서 스크립트로 적용
		// $(".Department input").attr("readonly",true); 
				 
    // Name : 사용자명. Department : 해당 사용자 부서, Position : 해당 사용자 직위, EmpNo : 해당 사용자 사번 (>> 직위는 안가져와지는 거 같음) 
    // $('.Name input, .Department input, .Position input, .EmpNo input').on('click', function(){
    $('.Name input').on('click', function(){
      self.employPopup($(this).closest('tr'));
    });
	},
		
  employPopup : function(trEl) {
    var self = this;
    $.goOrgSlide({
       header : '신청자 선택',
             isAdmin : false,
             type :'list', // list 설정시 사용자 선택
             ...
             callback : function(obj) {
                ...
                if (obj.id == undefined) {
                  $.goMessage('사용자를 선택해 주십시오');
                }
                
                $(trEl).find('.Name input').val(manager.name);
                $(trEl).find('.Department').text(manager.deptName);
                // $(trEl).find('.Department input').val(manager.deptName);
                $(trEl).find('.Position input').val(manager.position);
                $(trEl).find('.EmpNo input').val(manager.employeeNumber);
